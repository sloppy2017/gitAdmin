package com.fh.controller.manager.account;
...
import com.fh.service.manager.account.AccountManager;
...
public class AccountController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		mv.setViewName("manager/account/account_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("manager/account/account_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("账户名称");	//1
		titles.add("账户内容（地址／账号）");	//2
		dataMap.put("titles", titles);
		List<PageData> varOList = accountService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("ACCOUNT_NAME"));	    //1
			vpd.put("var2", varOList.get(i).getString("ACCOUNT_CONTENT"));	    //2
			varList.add(vpd);
		}
