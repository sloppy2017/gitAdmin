package com.fh.controller.manager.price;
...
import com.fh.service.manager.price.PriceManager;
...
public class PriceController extends BaseController {
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Price");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Price");
		...
		page.setPd(pd);
		List<PageData>	varList = priceService.list(page);	//列出Price列表
		mv.setViewName("manager/price/price_list");
		List<Dictionaries>	currencys = dictionariesService.listSubDictByParentId("b90241d67570474e905b58c6d18c9495"); //订单状态
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("manager/price/price_edit");
		List<Dictionaries>	currencys = dictionariesService.listSubDictByParentId("b90241d67570474e905b58c6d18c9495"); //订单状态
		mv.addObject("currencys", currencys);
		List<PageData>	projects = projectService.listAll(pd); //订单状态
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Price");
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Price到excel");
		...
		titles.add("项目Id");	//1
		titles.add("币种（1BTC，2ETH，3LTC，4USD，5RMB）");	//2
		titles.add("单价");	//3
		titles.add("创建时间");	//4
		titles.add("");	//5
		titles.add("");	//6
		dataMap.put("titles", titles);
		List<PageData> varOList = priceService.listAll(pd);
