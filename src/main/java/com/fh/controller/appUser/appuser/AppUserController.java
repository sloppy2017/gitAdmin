package com.fh.controller.appUser.appuser;
...
import com.fh.service.appUser.appuser.AppUserManager;
...
public class AppUserController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("APPUSER_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = appuserService.list(page);	//列出AppUser列表
		...
		mv.setViewName("appUser/appuser/appuser_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("appUser/appuser/appuser_edit");
		List<Dictionaries>	order_statuss = dictionariesService.listSubDictByParentId("yesOrNo"); //订单状态
		mv.addObject("order_statuss", order_statuss);
	...
	public Object deleteAll() throws Exception{
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			appuserService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("昵称");	//4
		titles.add("头像");	//5
		titles.add("用户状态(0不可用1可用)");	//6
		titles.add("创建时间");	//7
		dataMap.put("titles", titles);
		List<PageData> varOList = appuserService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var5", varOList.get(i).getString("USER_PIC"));	    //5
