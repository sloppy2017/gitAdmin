								...
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="项目名称" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="这里输入关键词"/>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">名额限制(人数)</th>
									
									<th class="center">状态</th>
									<th class="center">浏览数(个)</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.QUOTA_LIMIT}</td>
											
											<td class='center'>
												<c:forEach items="${project_states}" var="projectState">
													<c:if test="${projectState.NAME_EN == var.PROJECT_STATE}">${projectState.NAME }</c:if>
												</c:forEach>
											</td>
